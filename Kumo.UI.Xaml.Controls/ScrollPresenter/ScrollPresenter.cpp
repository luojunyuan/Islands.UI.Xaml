...
#include "InteractionTrackerOwner.ixx"
#include "ScrollPresenter.ixx"
#include "ScrollingScrollOptions.ixx"
#include "ScrollingZoomOptions.ixx"
#include "ScrollPresenterAutomationPeer.ixx"
#ifdef DBG
#include "ScrollPresenterTestHooks.ixx"
#endif
#include "Vector.ixx"
#include "RegUtil.ixx"
#include "MuxcTraceLogging.ixx"
...
winrt::float2 ScrollPresenter::ComputeEndOfInertiaPosition()
{
    if (m_state == winrt::ScrollingInteractionState::Inertia)
    {
        ...
        endOfInertiaPosition = winrt::float2{
            std::max(endOfInertiaPosition.x, minPosition.x),
            std::max(endOfInertiaPosition.y, minPosition.y)
        };
        endOfInertiaPosition = winrt::float2{
            std::min(endOfInertiaPosition.x, maxPosition.x),
            std::min(endOfInertiaPosition.y, maxPosition.y)
        };
    ...
    int32_t zoomFactorChangeCorrelationId)
{
    ...
    const int64_t distance = static_cast<int64_t>(std::abs(zoomFactor - m_zoomFactor));
    const winrt::Compositor compositor = winrt::ElementCompositionPreview::GetElementVisual(*this).Compositor();
