    ...
    double viewportLength)
{
    ...
    m_scrollBar.SmallChange(std::max(1.0, viewportLength / s_defaultViewportToSmallChangeRatio));
    ...
    const winrt::ScrollEventArgs& args)
{
    ...
    case winrt::ScrollEventType::ThumbTrack:
    {
        ...
        else
        {
            ...
            {
            case winrt::ScrollEventType::LargeDecrement:
                offsetChange = -std::min(m_lastScrollBarValue - m_scrollBar.Minimum(), m_scrollBar.LargeChange());
                break;
            case winrt::ScrollEventType::LargeIncrement:
                offsetChange = std::min(m_scrollBar.Maximum() - m_lastScrollBarValue, m_scrollBar.LargeChange());
                break;
            case winrt::ScrollEventType::SmallDecrement:
                offsetChange = -std::min(m_lastScrollBarValue - m_scrollBar.Minimum(), m_scrollBar.SmallChange());
                break;
            case winrt::ScrollEventType::SmallIncrement:
                offsetChange = std::min(m_scrollBar.Maximum() - m_lastScrollBarValue, m_scrollBar.SmallChange());
                break;
            }
